        ...
        <div class="search_style" style="padding-top: 10px;padding-left: 15px;">
            <div class="searchTable">
                ...
                <div class="layui-inline">
                    <input class="layui-input" name="search_keyword" id="search_keyword" autocomplete="off"
                           placeholder="name">
                    <input type="hidden" class="layui-input" name="searchColumn" id="searchColumn" autocomplete="off"
                           value="food_name">
                </div>
...
<script>
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#myData',
            id: 'myTable',
            url: 'FeedbackServlet?action=list',
                ...
                [
                    ...
                    <c:if test="${loginUser.userType == 'admin'}">
                    {
                    ...
                    },
                        {
                            ...
                            width: 130,
                            templet: vo => {
                                return (vo = vo.status === 'dealing') ? '<a class="layui-btn layui-btn-xs" lay-event="deal">deal</a>' : '';
                            }
                        }
                    </c:if>
                    <c:if test="${loginUser.userType == 'user'}">
                    {
                        ...
                        width: 130,
                        templet: vo => {
                            return '<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">del</a>';
                        }
                    }
                    </c:if>
                ]
        ...
        table.on('tool(myTable)', function (obj) { //注：tool 是工具条事件名，myTable 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                ...
                $.ajax({
                    type: "GET",
                    dataType: "text",
                    url: "FeedbackServlet?action=delete&id=" + data.id,
                    success: function () {
                        console.log("del！");
                    }
                });
                ...
                layer.open({
                    ...
                    fixed: false, //不固定
                    maxmin: true,
                    content: 'FeedbackServlet?action=editPre&id=' + data.id,
                ...
                layer.open({
                    ...
                    fixed: false, //不固定
                    maxmin: true,
                    content: 'FeedbackServlet?action=get&id=' + data.id
                });
                obj.update({});//同步更新缓存对应的值
                ...
                $.ajax({
                    type: "GET",
                    dataType: "text",
                    url: "FeedbackServlet?action=deal&id=" + data.id,
                    success: function () {
                        console.log("deal！");
                    }
                });
                obj.update({});
                layer.msg(data.foodName + ' deal success！');
        ...
        let $ = layui.$, active = {
            reload: function () {
                let url = 'FeedbackServlet?action=list&keyword=' + $('#search_keyword').val() + '&searchColumn=' + $('#searchColumn').val();
                //执行重载
